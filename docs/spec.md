...
- **outsideタグ**：旅行など、プロジェクトのタスクを進められない日に付けるトグル
  ...
  塗りつぶす
  - トグルON時、プロジェクト用メモ（上記の自由記述欄）に加えて**outside専用メモ**
    （`daily_notes.outside_content`）が現れ、2つの自由記述欄が画面分割（上下）で並ぶ。
    形式・操作性はプロジェクト用メモと全く同じRichTextEditor（太字・箇条書き・
    チェックリスト・ハイライト）で、保存先の列が異なるだけの完全に独立したメモ
  - このoutside専用メモに中身がある日は、タイムライン上部の日付ヘッダーのその日の列の
    上に折りたたみ可能な吹き出しが表示され、内容（チェックリストのチェック状態込み）を
    カレンダー上でひと目で確認できる。吹き出しはクリックで個別に開閉でき、開閉状態は
    ページを開いている間だけ保持（リロードで既定の展開状態に戻る）
...
| 2026-08-29 | ユーザー要望「ガントチャートのPhase内の各列をトグルで折りたたむ機能を追加」により、Phase表（`PhaseTable`）の列見出しに折りたたみボタン（`ChevronsLeft`/`ChevronsRight`）を追加。折りたたむと列幅を32pxまで縮め、セルの中身は描画しない（ラベル・削除ボタンも非表示、展開ボタンのみ残す）。折りたたみ状態は列id（`table_columns.id`）単位で`ui-store`の`collapsedPhaseTableColumnIds`にlocalStorage永続化する（ズーム率・タスク表サイズと同じ方式）。列幅ドラッグリサイズ（TanStack Tableの内部sizingキャッシュ）と競合しないよう、トグルのたびに`table.resetColumnSizing()`でキャッシュを破棄し、常に列定義側の幅（通常幅または折りたたみ幅）を使わせる |
| 2026-08-29 | ユーザー要望「サブタスクの文字を入力する欄が短すぎて入力しづらい。チェックボックス形式はそのままで、備考欄と同じような入力形式にして欲しい」により、`SubtaskListCell`の項目文字入力を、1行の`Input`＋クリックで編集モードに入る方式（`InlineEditableText`）から、備考・タスク名セル（`TextCell`）と同じ「常時編集可能・入力行数に合わせて自動で高さが伸縮するtextarea」方式に変更。共通処理を`AutosizeTextarea`（`features/shared`）として切り出し、既存項目の編集欄と新規追加欄の両方に使用した。チェックボックスの見た目・操作は変更していない |
| 2026-08-29 | ユーザー報告「表からドラッグして、セルにタスクを埋める時、カーソルをあわせているセルの2つ前の日付にタスクが埋め込まれてしまう」を修正。表の行→日付セルへのドラッグ登録（`phase-row.tsx`のDndContext）の衝突判定を`closestCenter`から`pointerWithin`に変更した。`closestCenter`は表内の小さなグリップアイコンの元rectをポインタ移動量ぶん平行移動した矩形の中心を基準に最近傍のセルを判定するため、掴んだ位置とDragOverlayで見えるチップの見た目の位置がズレていると実際のカーソル位置とは異なるセルに判定されてしまっていた。行の並び替え（SortableContext）側は従来どおり`closestCenter`のまま |
| 2026-08-29 | ユーザー報告に基づき、以下3件のパフォーマンス・見た目の不具合を修正。(1)「表の読み込みが遅い」「Phase表（⊞ボタン）を開いた瞬間」：`NoteRichCell`（備考セル）が行数分すべて同時にTiptapエディタを生成しており重かったため、クリック/フォーカスするまでは軽量なプレーンテキストのプレビュー表示に留め、実際に触るセルだけ遅延生成するよう変更（`RichTextEditor`に`autoFocus`オプションを追加し、遅延生成時もクリック直後から続けて入力できるようにした）。(2)「チェック・文字入力などの操作時」が遅い：`useUpdateCell`（セル更新共通）に楽観的更新を追加し、サーバー応答を待たずキャッシュを即時書き換えるよう変更。あわせて、セルの値更新では行の集合が変わらないのに`tableRows`まで無効化していた無駄な再取得（`tableCells`と二重に発生していた）をやめ、`tableCells`のみを対象にした。(3)「カレンダー上でドラッグ中のタスクを移動させると、灰色の点灯と、黒の点灯が二重でブレる」：上記(2)のpointerWithin採用後、日付セルのネイティブ`:hover`（ユーザーの実カーソル位置に追従）とdnd-kitの`isOver`（衝突判定確定後に1テンポ遅れて切り替わる）が同時に効いて隣接セルが一瞬同時に光っていたため、ドラッグ中は`:hover`のハイライトを止め`isOver`のみを表示するようにした |
...
| 2026-09-02 | ユーザー要望「outsideをオンにするとoutside用のタスク欄を追加される。そして、outsideのタスクが追加された日は、カレンダーの日付上に吹き出しでoutsideのタスクが表示される。この吹き出しは、折りたたみ可能」（`outsideについて.png`にレイアウト参照あり）により、outsideタグ機能を拡張。(1) `daily_notes`に新カラム`outside_content`（jsonb、Tiptap doc、`supabase/migrations/0011_outside_tasks.sql`）を追加し、outsideトグルON時にのみプロジェクト用メモとは別のoutside専用メモ（`RichTextEditor`）を表示するようにした。(2) この欄に中身がある日付を`fetchOutsideContentNotes`で取得し（`useOutsideContentNotes`、`timeline-context.tsx`経由で配布）、日付ヘッダー（`TimelineHeader`）の月ラベル行の上に「吹き出しレーン」を追加。CSS Gridの列幅を日付セルと完全一致させることで各吹き出しを対象日の列の真上に配置し、下端の短い縦線（テール）でその日を指し示す。吹き出しはクリックで個別に開閉でき（`OutsideTaskCallouts`のローカルstate、開閉状態はページを開いている間だけ保持） |
| 2026-09-02 | 上記の対応が意図と異なっていたため（ユーザー「outsideのタスク欄ですが一行ずつタスクを記入する形式ではなく、プロジェクト用のメモ欄と全く同じ形式にして。outsideとプロジェクトで、メモ欄が画面分割されているイメージです」）、専用タスク欄をチェックボックス付きリスト（1行1タスク）からプロジェクト用メモと全く同じ自由記述欄（`RichTextEditor`、太字・箇条書き・チェックリスト・ハイライト）に作り直した。`daily_notes.outside_tasks`（jsonb配列）を`outside_content`（jsonb、Tiptap doc）に置き換え（列はまだ本番に未適用だったため、マイグレーション自体を書き換えて対応）、サイドバーはoutsideトグルON時、プロジェクト用メモとoutside専用メモを上下に画面分割して両方同時に表示する構成にした。カレンダー吹き出しのプレビューは、既存のObsidianコピー用Markdown変換（`docToMarkdown`）を再利用し、行頭の記法だけ外して箇条書き表示にする |
