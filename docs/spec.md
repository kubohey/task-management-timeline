...
| 2026-08-29 | ユーザー報告「引っ張ったら元の位置に戻ってしまう」により、直前の列幅ドラッグ同期修正の副作用を修正。`useUpdateRoadmapColumn`に楽観的更新（`onMutate`でキャッシュを即時書き換え、`onError`でロールバック）を追加し、ドラッグ確定直後にDBからの応答が返るまでの間だけ幅が古い値へ一瞬戻って見える不具合を解消（`useReorderPhases`と同じパターン）。あわせて、タスクブロックのラベル（テキストエリア）を中央揃えに変更 |
